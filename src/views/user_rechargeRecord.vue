          ...
          <div class='noticeBox' v-for="(item, index) in articleList" :key="index">
            <div class='notices'>
              ...
              <div class='noticeText'>
                <div class='noticeTitle'>{{item.kind_name}}</div>
                <!--                  <div class='noticeTitle'>工号:{{item.emp_id}}</div>-->
                <div class='noticeContent'>{{item.op_date}}</div>
              </div>
            </div>
            <div class='more'>
              +{{item.charge_money}}
...
<script>
  ...
  import {queryMchargerecordsList} from '@/api/index/index';
        ...
        queryMchargerecordsList(reqData).then(res => {
          ...
          if (result.length > 0) {
            result.forEach((item, index) => {
              //充值类型 1：现金充值 2:补贴  6：出纳充值 8：清零补贴 20：补领余额 99：微信充值'
              item['index'] = index + 1;
              const {kind} = item
              if (kind == 1) {
                item['kind_name'] = '现金充值'
