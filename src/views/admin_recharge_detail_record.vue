<template>
  <div class="content">
      ...
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
        <van-list v-model="loading"
          ...
          <van-cell v-for="(item,index) in list" :key="item.id">
            <van-col span="2"><img src="../assets/images/tabImg/recharge.png" style="width: 40px"/></van-col>
            <van-row type="flex" justify="space-between">
              <van-col span="12" offset="2">{{item.op_date}}</van-col>
              <van-col span="10">金额: +{{item.charge_money}}</van-col>
            </van-row>
            <van-row type="flex" justify="space-between">
              <van-col span="12" offset="2">工号: {{item.emp_id}}</van-col>
              <van-col span="10" >卡号: {{item.card_id}}</van-col>
            </van-row>
          </van-cell>
        </van-list>
      </van-pull-refresh>
    ...
    <van-popup v-model="show"
      ...
      <van-cell-group>
        <van-field label="充值日期" :value="date" @click="dateShow = true" readonly
...
<script>
  ...
  import {queryMchargerecordsList} from '@/api/index/manager';
  ...
  export default {
    ...
    mounted() {
      ...
      this.queryMchargerecordsList(reqData);
    ...
    methods: {
      ...
      onSearch() {
        ...
        this.queryMchargerecordsList(data);
        this.show=false;
      },
      ...
      /** 获取充值记录列表 **/
      queryMchargerecordsList(reqData) {
        queryMchargerecordsList(reqData).then(res => {
