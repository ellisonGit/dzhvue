...
<script>
  ...
  import {queryDayTermMealSumList} from '@/api/index/manager';
  ...
  export default {
    ...
    methods: {
      ...
      getArticleList(reqData) {
        queryDayTermMealSumList(reqData).then(res => {
      ...
      onRefresh(done) {
        ...
        queryDayTermMealSumList(reqDate).then(res => {
          const {data, code, message} = res.data;
          const resultData = JSON.parse(data);
          if (code !== 200) {
            this.$vux.toast.show({
