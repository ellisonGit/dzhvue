<template>
  <div class="content">
    <div class="nav-top">
      <van-nav-bar title="卡余额信息" left-arrow @click-left="onClickLeft()">
        <template #right>
      ...
      <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
        <van-list v-model="loading"
          ...
          <van-cell v-for="(item,index) in list" :key="item.id">
            <van-col span="2"><img src="../assets/images/admin/balance.png" style="width: 40px"/></van-col>
            <van-row type="flex" justify="space-between">
              <van-col span="12" offset="2">{{item.sign_time}}</van-col>
              <van-col span="10">余额: {{item.card_balance}}</van-col>
            </van-row>
            <van-row type="flex" justify="space-between">
    ...
    <van-popup v-model="show"
      ...
      <van-cell-group>
        <van-field label="消费/充值日期" :value="date" @click="dateShow = true" readonly
...
<script>
  ...
  import {queryCardBalanceList} from '@/api/index/manager';
  ...
  export default {
    ...
    mounted() {
      ...
      this.queryCardBalanceList(reqData);
    ...
    methods: {
      ...
      onSearch() {
        ...
        this.queryCardBalanceList(data);
        this.show=false;
      },
      ...
      /** 获取卡余额列表 **/
      queryCardBalanceList(reqData) {
        queryCardBalanceList(reqData).then(res => {
